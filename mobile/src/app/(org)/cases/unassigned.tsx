...
import { Search, WifiOff, X, HelpCircle } from 'lucide-react-native';
...
export default function UnassignedCasesScreen() {
  const { user } = useAuth();
  ...
  const load = useCallback(async () => {
    if (!user) return;
    try {
      const response = await allocationsApi.listAllocations({ status: 'UNASSIGNED', size: 100 });
  ...
  if (loading) return <LoadingView label="Loading pool cases…" />;

  return (
    <Screen edges={['top']}>
      <View style={{ gap: spacing.s4, paddingBottom: spacing.s4 }}>
        <View>
          <Text variant="title">Unassigned Cases</Text>
          <Text variant="caption" color="secondary">{cases.length} cases in pool waiting dispatch</Text>
        </View>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            loadError ? (
              <EmptyState icon={WifiOff} title="Couldn't load unassigned pool" message="Pull down to try again." />
            ) : (
              <EmptyState
                icon={HelpCircle}
                title="No cases in pool"
                message={search ? 'Try adjusting your search.' : 'All allocations have been dispatched.'}
              />
            )
          }
        />
